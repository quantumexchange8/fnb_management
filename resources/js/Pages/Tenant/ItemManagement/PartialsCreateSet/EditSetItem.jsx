...
export default function EditSetItem({ data, setData, errors, getErrors }) {
                ...
                label: (
                    <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2">
                            <div className={`${partiallyChecked ? 'text-primary-500' : 'text-neutral-800'} text-neutral-800 text-base font-bold`}>{category.name}</div>
                        </div>
                    </div>
                ),
                children: (
                    <div className="space-y-3">
                        {
                            filteredProducts.length > 0 ? (
                                filteredProducts.map((product, pIndex) => (
                                    <div key={pIndex} className="flex flex-col border rounded-lg">
                                        {/* product card here */}
                                        <div className="flex items-center gap-4 p-2">
                                            {
                                                product.product_image ? (
                                                    ...
                                                    <div className="font-bold text-wrap text-xs w-[52px] h-[52px] flex justify-center items-center bg-neutral-50 rounded-lg">
                                                        {product.item_code}
                                                    </div>
                                                )
                                            }
    ...
    const removeFromFixedItem = (removeId) => {
        
        setData(prev => ({
            ...prev,
            fixed_item: (prev.fixed_item || []).filter(option => option.id !== removeId)
        }));

        const updatedMealItem = getMealItem.map(cat => {
            const updatedProducts = cat.product.map(prod => {
                if (prod.id === removeId) {
                    return {
                        ...prod,
                        checked: false,
        ...
        setData('deleted_fixed_item', [removeId]);

        setGetMealItem(updatedMealItem);
    }
    ...
    const editSelectableGroup = (group) => {
        setIsEditSelectableGrpOpen(true);
        setEditSelectableItem(group);
    }
    const closeEditSelectableGroup = () => {
        setIsEditSelectableGrpOpen(false);
        setEditSelectableItem(null);
    }
    ...
    return (
        <>
                                    ...
                                    <AnimatePresence mode="popLayout">
                                        {
                                            displayedGroups.map((group, i) => (
                                                ...
                                                >
                                                    ...
                                                    <motion.div
                                                        layout
                                                        className="p-4 border border-neutral-50 bg-white shadow-sec-voucher w-full flex items-center rounded-xl cursor-pointer hover:bg-neutral-25"
                                                        onClick={() => editSelectableGroup(group)}
                                                    >
                    ...
                    <div className="py-5 flex flex-col gap-5">
                        ...
                        <div className="flex items-center gap-5 px-5">
                            ...
                            <div className="w-full">
                                <InputNumber 
                                    prefix='RM '
                                    value={(data.base_price ?? 0 ) || (data.price_setting === 'sum_item' && 0)}
                                    min={0.00}
                                    step="0.01"
            ...
            <SelectableGroup 
                ...
                setIsSelectableGrpOpen={setIsSelectableGrpOpen} 
                closeGroup={closeGroup} 
                editSelectableItem={editSelectableItem}
                isEditSelectableGrpOpen={isEditSelectableGrpOpen}
